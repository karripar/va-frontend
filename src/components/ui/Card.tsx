    ...
    <div className="flex gap-8 flex-col my-2">
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div
            key={i}
            className={`relative border border-[var(--va-grey-50)] rounded-lg bg-[var(--va-card)] shadow-md hover:shadow-lg transition-shadow p-2 text-left ${
              ...
              <>
                ...
                {step.links && (
                  <div className="w-max-full sm:mx-6 mx-1 flex flex-col justify-center">
                    {step.links.map((link) => {
                      // check if the link exists in the database links
                      const dbLink = links.find(
                        (l) => l.label === link.label && l.stepIndex === i
                      );

                      const href = (dbLink?.href as string) || link.href;
                      const isExternal =
                        (dbLink?.isExternal as boolean) !== false;
                      const isFile = (dbLink?.isFile as boolean) === true;
                      const linkKey = `${i}-${link.label}`;
                      ...
                      return (
                        <div key={link.href} className="flex flex-col">
                          {!isEditing && (
                            <div className="flex items-center sm:gap-2 gap-1">
                              ...
                              {isAdmin && (
                                <button
                                  onClick={() =>
                                    handleStartEditLink(i, link.label, href)
                                  }
                          ...
                          {/* edit links */}
                          {isAdmin && isEditing && (
                            <div className="bg-[var(--va-grey-50)] p-4 sm:p-8 rounded-lg space-y-3 my-3">
                              <label
                                style={{
                                  fontFamily: "var(--font-machina-bold)",
                                }}
                                className="text-md text-[var(--va-orange)] block"
                              >
                                {link.label}
                              </label>
                              <div>
                              ...
                              <div>
                                <label className="text-sm text-[var(--va-dark-grey)] block my-4 pt-2">
                                  Lisää uusi osoite tai lataa tiedosto:
                                </label>
                                <input
                                  type="text"
                                  onChange={(e) =>
                                ...
                                <div className="bg-white p-4 rounded border border-dashed border-[var(--va-orange-50)] hover:border-[var(--va-orange)] transition-colors my-6">
                                  <div className="flex flex-col items-center mx-2">
                                    ...
                                    <p className="my-2 text-sm text-[var(--va-typography)] px-4">
                                      Hyväksytyt tiedostotyypit: PDF, PPT, PPTX,
                                      DOC, DOCX
                                    </p>
                                    <div className="my-4 w-full sm:px-6">
                                      <input
                                        ...
                                        className="hidden"
                                        id={`file-upload-${i}-${link.label}`}
                                      />
                                      <label
                                        htmlFor={`file-upload-${i}-${link.label}`}
